...
## Recent Changes
- 2026-03-04: **PWA full setup & offline mode**: Service worker `vem-v6` with 3-tier caching (static, API, navigation); offline HTML fallback page with 3-language tabs (auto-detects user's saved locale from `vem-locale` localStorage key); OfflineBanner React component (full-screen overlay with animated WifiOff, gradient orbs, auto-reconnect via setInterval every 3s + online event); manifest.json updated; top referrers now count only VIP-purchasing referrals

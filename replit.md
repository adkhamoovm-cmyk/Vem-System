...
## Recent Changes
- 2026-03-22: **Professional Landing Page + Auth Modal**: New marketing landing page at "/" replacing the redirect to /login. Sections: Hero with gradient title, Partner Brands (Netflix/Amazon/HBO/Disney+/Apple TV+/Hulu), 4 Features cards, 3-step "How It Works", 6-item FAQ accordion (including digital documents explanation), CTA section, Footer with Telegram links. Auth (login/register) now opens in a modal dialog on the landing page, reusing existing form components unchanged. All 5 languages updated with `landing.*` keys (48 keys each). Old /login and /register routes still work as standalone pages.

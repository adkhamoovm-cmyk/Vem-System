...
## Recent Changes
- 2026-03-22: **Professional Landing Page + Auth Modal**: New marketing landing page replacing the old redirect-to-login flow. Routes: "/" shows landing page, "/login" shows landing + login modal auto-opened, "/register" shows landing + register modal auto-opened. Sections: Hero with gradient title, Partner Brands (Netflix/Amazon/HBO/Disney+/Apple TV+/Hulu), 4 Features cards, 3-step "How It Works", 6-item FAQ accordion (including digital documents explanation), CTA section, Footer with Telegram links. Auth (login/register) opens in a modal dialog reusing existing LoginForm/RegisterForm components unchanged. URL syncs when opening/closing modal. Standalone AuthPage removed. All 5 languages updated with `landing.*` keys (48 keys each).
- 2026-03-10: **Security audit fixes**: 1) Receipt uploads protected with auth middleware — `/uploads/receipts` now requires session login; 2) XSS vulnerability fixed in `linkifyText()` — quotes/apostrophes escaped, URL regex tightened, `href` attribute sanitized; 3) Server message translation coverage expanded — 35+ new `serverMessageMap` entries (PIN, sessions, investments, admin messages) with translations in all 5 languages; 35 new `server.*` keys added to uz/en/ru/tr/es locale files
- 2026-03-10: **Sessions modal redesign**: Professional UI with gradient header, active session count badge, Wifi/Clock tab icons, cyan gradient for current session, online status indicator, relative time (localized), color-coded history entries, 6 new i18n time keys in 5 languages
- 2026-03-10: **FAQ timezone update**: Withdrawal hours in faqA3 now show times for 5 countries (USA EST 01:00-07:00, Turkey 09:00-15:00, Germany 07:00-13:00, Russia MSK 09:00-15:00, Uzbekistan 11:00-17:00) across all 5 locales
- 2026-03-05: **Exchange UID fast transfer system**: When user selects Binance or Bitget as exchange, modal asks for UID + email (for fast internal transfer). Green hint box explains "UID transfer is secure and instant — bypasses blockchain". If user tries to skip, amber warning "This is a one-time opportunity" appears. If declined, falls back to BEP20 address only. New DB fields: `exchange_uid`, `exchange_email` on `payment_methods`. Admin withdrawal view shows UID/email with "Fast transfer" badge. User wallet card shows UID/email if present. All 5 locales updated (uz/ru/en/es/tr).
- 2026-03-04: **Notification retranslation for legacy data**: Frontend `notifications.tsx` has `NOTIF_TEMPLATES` (13 types × 5 langs) + `extractParams` regex engine + `retranslateText` — eski bazadagi 3 tildagi (uz/ru/en) notificationlarni real-time es/tr ga tarjima qiladi parametrlarni saqlab
- 2026-03-04: **Spanish & Turkish locales fully integrated**: `es.json` and `tr.json` locale files (300+ keys each); language switcher updated with 🇪🇸/🇹🇷 flags; SW offline page updated with ES/TR tabs and messages; SW cache bumped to `vem-v7`/`vem-static-v7`/`vem-api-v4`
- 2026-03-04: **PWA full setup & offline mode**: Service worker `vem-v7` with 3-tier caching (static, API, navigation); offline HTML fallback page with 5-language tabs (auto-detects user's saved locale from `vem-locale` localStorage key); OfflineBanner React component (full-screen overlay with animated WifiOff, gradient orbs, auto-reconnect via setInterval every 3s + online event); manifest.json updated; top referrers now count only VIP-purchasing referrals
- 2026-03-04: **Bug audit fixes**: Sunday check aligned with 03:00 UZB day boundary (uses getUzbDayNow instead of getUzbRealNow for day-of-week); VIP purchase 4 separate UPDATE queries merged into 1 atomic SET; cron interval reduced from 24h to 1h (prevents missed profits on server restart); composite index on task_history(user_id, video_id, completed_at) for faster duplicate checks
- 2026-03-04: **Timezone centralization & 03:00 UZB reset**: All daily boundaries (task reset, fund profit, withdrawal daily limits, duplicate video check) now use centralized helpers in `server/lib/helpers.ts`: `getUzbDayNow()` (Date with +2h offset = UZB+5 minus 3h reset), `getUzbToday()` (date string), `getUzbRealNow()` (actual UZB time for clock-based checks like Sunday/withdrawal hours), `DAY_OFFSET_MS`, `DAY_RESET_SQL_INTERVAL`. Replaced inline `uzbOffset = 5 * 60 * 60 * 1000` in 4 route files (auth, user, financial, admin). Day boundary shifts from midnight UZB to 03:00 UZB (03:00 Toshkent = 22:00 UTC previous day). Only `DAY_RESET_HOUR` constant needs changing to adjust reset time globally.
- 2026-03-03: **Race condition hardening**: T001: promo code usage fully inside db.transaction with SELECT FOR UPDATE on promo row + usage check inside tx + unique DB index (uq_promo_user) on (promoCodeId, userId); T002: task completion daily limit check + alreadyWatched check both inside db.transaction with SELECT FOR UPDATE on user row, atomic dailyTasksCompleted; T003: admin balance set already uses SELECT FOR UPDATE; i18n: nav.notifications key added to all 3 locales; tasks page loading skeleton guard before Sunday/VIP checks
- 2026-03-03: **Financial security hardening**: DB CHECK constraint `balance_non_negative` (balance >= 0); fund/invest endpoint wrapped in db.transaction (balance deduction + investment creation + first profit all atomic); withdrawal endpoint wrapped in db.transaction (balance deduction + withdrawal record + history all atomic); setupDailyProfits cron wrapped in db.transaction per investment (profit + principal return + status update all atomic); notifications moved outside transactions to avoid blocking
- 2026-03-03: **Code quality pass 2**: auth.tsx split into 6 sub-components (slider-captcha, login-form, register-form, reset-password-modal, terms-modal, shared) → orchestrator ~130 lines; dashboard.tsx split into 7 sub-components (auto-scroll-carousel, hero-section, balance-card, tasks-widget, quick-actions, video-grids, install-app-modal) → orchestrator ~360 lines; ~60 `any` types replaced with proper TypeScript interfaces; Zod validation added to ALL admin/user/financial routes (adminSchemas, userSchemas, financialSchemas); asyncHandler wrapper + global error handler eliminates repetitive try/catch; `validateBody()` middleware on ~25 POST routes; all Zod schemas have `required_error` with O'zbekcha xabar; `validateBody()` fallback maps generic "Required"/"Invalid input" → "Barcha maydonlarni to'ldiring"
...
### Frontend
- `client/src/pages/admin.tsx` (183 lines) - Admin orchestrator, imports 13 sub-components from admin/

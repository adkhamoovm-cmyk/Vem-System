...
## Recent Changes
- 2026-03-04: **PWA full setup & offline mode**: Service worker upgraded with 3-tier caching (static, API, navigation); offline HTML fallback page in Uzbek with animated UI; OfflineBanner React component (red gradient top bar with WifiOff icon, auto-detects online/offline events); manifest.json updated (start_url=/dashboard, lang=uz, categories); viewport-fit=cover for iOS notch support; safe-area-inset-top padding on offline banner; default country code changed from +49 DE to +998 UZ; top referrers now count only VIP-purchasing referrals (INNER JOIN users WHERE vipLevel > 0)

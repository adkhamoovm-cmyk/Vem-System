...
router.post("/api/auth/reset-step1", authRateLimiter, async (req: Request, res: Response) => {
  try {
...
router.post("/api/auth/reset-step2", authRateLimiter, async (req: Request, res: Response) => {
  try {
...
router.post("/api/auth/reset-step3", authRateLimiter, async (req: Request, res: Response) => {
  try {
...
router.post("/api/auth/reset-password", authRateLimiter, async (req: Request, res: Response) => {
  try {
...
router.post("/api/admin/verify-pin", pinRateLimiter, requireAuth, async (req: Request, res: Response) => {
  try {
    const user = await storage.getUser(req.session.userId!);
...
router.get("/api/admin/pin-status", requireAuth, async (req: Request, res: Response) => {
  try {
    const user = await storage.getUser(req.session.userId!);
    if (!user || !user.isAdmin) {
      return res.status(403).json({ message: "Admin huquqi talab qilinadi" });
...
router.post("/api/admin/change-pin", pinRateLimiter, requireAdmin, async (req: Request, res: Response) => {
  try {

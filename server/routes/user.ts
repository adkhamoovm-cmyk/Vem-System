...
import { requireAuth, taskRateLimiter, withdrawRateLimiter, sendNotification, hashPassword, comparePasswords, uploadAvatar } from "../lib/helpers";

const router = Router();

router.get("/api/videos", requireAuth, async (_req: Request, res: Response) => {
  try {
...
router.post("/api/tasks/complete", requireAuth, taskRateLimiter, async (req: Request, res: Response) => {
  try {
...
router.get("/api/vip-packages", requireAuth, async (_req: Request, res: Response) => {
  try {
...
router.post("/api/vip/purchase", requireAuth, withdrawRateLimiter, async (req: Request, res: Response) => {
  try {
    const userId = req.session.userId!;
...
router.get("/api/referrals/stats", requireAuth, async (req: Request, res: Response) => {
  try {
...
router.get("/api/referrals/users", requireAuth, async (req: Request, res: Response) => {
  try {
...
router.get("/api/referrals/extended-stats", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.session.userId!;
...
router.post("/api/profile/change-password", requireAuth, async (req: Request, res: Response) => {
  try {
...
router.post("/api/profile/change-fund-password", requireAuth, async (req: Request, res: Response) => {
  try {
...
router.post("/api/profile/avatar", requireAuth, uploadAvatar.single("avatar"), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
...
router.get("/api/my-sessions", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.session.userId!;
...
router.post("/api/my-sessions/terminate", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.session.userId!;
    const { sid } = req.body;

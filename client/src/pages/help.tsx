  ...
  return (
    <AppLayout>
      <div className="p-4 space-y-4">
        <div className="bg-gradient-to-br from-[#3B82F6] via-[#2563EB] to-[#1d4ed8] rounded-2xl p-5 text-white shadow-lg relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-white/70 text-xs">Savollaringizga javob topamiz</p>
            </div>
          </div>
        ...
        {helpSections.map((section, i) => (
          <div key={i} className="bg-[#1a1a1a] rounded-2xl border border-[#2a2a2a] overflow-hidden" data-testid={`help-section-${i}`}>
            <div className="px-4 py-3.5 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[#666] text-[10px]">{section.description}</p>
              </div>
            </div>
              ...
              {section.items.map((item, j) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <item.icon className="w-4 h-4 text-[#555]" />
                    <div>
                      <p className="text-[#ddd] text-sm">{item.label}</p>
                      <p className="text-[#888] text-xs">{item.value}</p>
                    </div>
                  </div>
        ...
        <div className="bg-[#1a1a1a] rounded-2xl border border-[#2a2a2a] overflow-hidden">
          <div className="px-4 py-3.5 flex items-center gap-3">
            ...
            <div>
              <h3 className="text-white font-semibold text-sm">Ko'p so'raladigan savollar</h3>
              <p className="text-[#666] text-[10px]">FAQ</p>
            </div>
          </div>
          <div className="border-t border-[#222] divide-y divide-[#222]">
            {faqItems.map((faq, i) => (
              <div key={i} className="px-4 py-3" data-testid={`faq-item-${i}`}>
                <p className="text-white text-xs font-semibold mb-1">{faq.q}</p>
                <p className="text-[#888] text-[11px] leading-relaxed">{faq.a}</p>
              </div>
            ))}
          </div>
        </div>

...
import { ArrowLeft, TrendingUp, Users, Briefcase, Gift, Shield, Calendar, ChevronDown, DollarSign, BarChart3 } from "lucide-react";
...
export default function ReportsPage() {
  ...
  const categoryItems = [
    { key: "task" as const, label: "Vazifalar daromadi", icon: TrendingUp, color: "from-emerald-500 to-green-600", textColor: "text-emerald-500", bgColor: "bg-emerald-500/10" },
    { key: "referral" as const, label: "Agent daromadi (Referal)", icon: Users, color: "from-blue-500 to-cyan-600", textColor: "text-blue-500", bgColor: "bg-blue-500/10" },
    { key: "fund" as const, label: "Investitsiya daromadi", icon: Briefcase, color: "from-violet-500 to-purple-600", textColor: "text-violet-500", bgColor: "bg-violet-500/10" },
    { key: "promo" as const, label: "Promokod daromadi", icon: Gift, color: "from-amber-500 to-orange-600", textColor: "text-amber-500", bgColor: "bg-amber-500/10" },
    { key: "admin" as const, label: "Texnik bo'lim", icon: Shield, color: "from-rose-500 to-red-600", textColor: "text-rose-500", bgColor: "bg-rose-500/10" },
  ];
      ...
      <div className="sticky top-0 z-30 bg-background/95 backdrop-blur-md border-b border-border/50">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1">
            <h1 className="text-foreground font-bold text-lg">Hisobot</h1>
            <p className="text-muted-foreground text-xs">Daromadlar hisoboti</p>
          </div>
          ...
          <>
            <div className="bg-gradient-to-br from-primary/10 via-blue-500/5 to-violet-500/10 rounded-2xl p-5 border border-primary/20">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-muted-foreground text-xs font-medium">
                  {period === "1" ? "Bugungi" : period === "7" ? "Haftalik" : period === "30" ? "30 kunlik" : "Umumiy"} jami daromad
                </span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-card rounded-xl p-3 border border-border/50 text-center">
                <p className="text-muted-foreground text-[10px] mb-1">Bugun</p>
                <p className="text-emerald-500 font-bold text-lg">${report.today.toFixed(2)}</p>
              </div>
              <div className="bg-card rounded-xl p-3 border border-border/50 text-center">
                <p className="text-muted-foreground text-[10px] mb-1">Kecha</p>
                <p className="text-blue-500 font-bold text-lg">${report.yesterday.toFixed(2)}</p>
              </div>
              <div className="bg-card rounded-xl p-3 border border-border/50 text-center">
                <p className="text-muted-foreground text-[10px] mb-1">Hafta</p>
            ...
            <div className="bg-card rounded-2xl p-4 border border-border/50">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-foreground text-xs font-bold">Kunlik grafik</span>
              </div>
            ...
            <div className="bg-card rounded-2xl border border-border/50 overflow-hidden">
              <div className="px-4 py-3 border-b border-border/30">
                <span className="text-foreground text-xs font-bold">Daromad turlari</span>
              </div>
              ...
              <div className="px-4 py-3 border-t border-border/50 bg-muted/20 flex items-center justify-between">
                <span className="text-foreground text-xs font-bold">Jami</span>
